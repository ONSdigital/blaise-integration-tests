...
namespace Blaise.Api.Tests.Behaviour.Helpers
{
    public class CaseDataHelper
    {
        ...
        public string GetFieldValue(string primaryKey, FieldNameType fieldNameType)
        {
            var dataField = _blaiseApi.GetFieldValue(_connectionModel, primaryKey, _instrumentName, _serverPark,
                fieldNameType);

            return dataField.ValueAsText;
        }
        ...
        private static Dictionary<string, string> MapFieldDictionaryFromRecordFields(IDataRecord2 recordData)
        {
